...
class AllocationController extends Controller
{
    ...
    function fillRemainingQuotaVacanciesWithTemporaryAllocatedCandidates_OLD(): void
    {
        ...
        DB::transaction(function () {
            ...
            */
            ...
            /*
            |--------------------------------------------------------------------------
            | 2. Load temporary allocated candidates
            |--------------------------------------------------------------------------
            */
            $candidates = DB::table('candidates')
                ->where('allocation_status', '=', 'temporary')
            ...
            /*
            |--------------------------------------------------------------------------
            | 3. Process CADRE by CADRE
            |--------------------------------------------------------------------------
            */

            $shifting_allocation_count = 0;
                ...
                foreach (['CFF','EM','PHC'] as $quota) {
                    ...
                    while ($post->$quotaCol > 0) {
                        ...
                        */
                        $eligible = $candidates->filter(function ($c) use ($post, $quota) {
                            ...
                            foreach ($choices as $item) {
                                ...
                                if( $item === trim($this->get_cadre_abbr_by_code($c->assigned_cadre)) ) 
                                {
                                    $found = true;
                                    break;
                                }
                            ...
                            $rightPartOfChoiceList = trim(implode(' ', $left));

                            if (!in_array($post->cadre_abbr, $left)) return false;
                            ...
                            // Other cadre than GENERAL
                            if( $post->cadre_type !== 'GG' ) {
                                return !empty($c->technical_merit_position);
                            }

                            //Check technical passed cadre eligibility: TECHNICAL cadre abbr must exists
                        ...
                        */
                        ...
                        $currentlyAllocated = $winner->assigned_cadre;
                        $currentlyAssignedStatus = $winner->assigned_status;

                        DB::table('candidates')
                            ->where('id', $winner->id)
                            ->update([
                                'assigned_cadre' => $post->cadre_code,
                                'assigned_status' => 'SHIFTED-UPPER',
                                'allocation_status'  => 'temporary',
                                'general_status'  => 'Shifted from lower allocated '. $currentlyAllocated .' cadre to higher choice quota vacant post.',
                            ]);

                        
                        //Adjust Previously Allocated Cadre Post Count   
                        DB::table('posts')
                            ->where('cadre_code', $currentlyAllocated)
                            ->incrementEach([
                                $currentlyAssignedStatus . '_post_left' => 1, 
                                'total_post_left' => 1
                            ]);

                        DB::table('posts')
                            ->where('cadre_code', $currentlyAllocated)
                            ->decrement('allocated_post_count');

                        //Adjust Updated Cadre Post Count 
                        DB::table('posts')
                            ->where('id', $post->id)
                            ->decrementEach([
                                $quotaCol => 1, 
            ...
            echo 'Status: Total Shifting = '. $shifting_allocation_count .'<br><br>';

        });

        
        echo 'Status: Shifting Allocation Done!<br>';

    } //End of function 'fillRemainingQuotaVacanciesWithTemporaryAllocatedCandidates()'



    /**Fill up Quota post from NON-ALLOCATED candidates, if possible */
    function fillRemainingQuotaVacanciesWithNationalMerit(): void
    {
        ...
        DB::transaction(function () {
            ...
            */
            ...
            foreach ($posts as $post) {
                ...
                foreach (['MQ','CFF','EM','PHC'] as $quota) {
    ...
    } //End of function 'fillRemainingQuotaVacanciesWithNationalMerit()'
